...
ConsistenciaException {
 ...
 Optional<List<Cartao>> cartoes =
Optional.ofNullable(cartaoRepository.findByClienteId(clienteId));
 if (!cartoes.isPresent() || cartoes.get().size() < 1) {
 log.info("Service: Nenhum cartão encontrado para o cliente de id: {}",
clienteId);
 ...
 if
(!clienteRepository.findById(cartao.getCliente().getId()).isPresent()) {
 log.info("Service: Nenhum cliente com id: {} foi encontrado",
